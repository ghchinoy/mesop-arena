...
def history_page_content(app_state: me.state):
    ...
    with page_scaffold():  # pylint: disable=not-context-manager
        with page_frame():  # pylint: disable=not-context-manager
            ...
            with me.box(
                style=me.Style(display="flex", flex_direction="column", align_items="center", width="90hv")
            ):
                for v in votes:
                    model1 = v.get('model1')
                    image1 = v.get('image1')
                    ...
                    with me.box(
                        ...
                        with me.box(
                            ...
                            with me.box(style=me.Style(flex_direction="column", display="flex", align_items="center")):
                                me.image(src=image1_url, style=WINNER_THUMBNAIL_STYLE if winner == model1 else THUMBNAIL_STYLE)
                                me.text(model1, style=me.Style(font_size="10pt"))
                            with me.box(style=me.Style(flex_direction="column", display="flex", align_items="center")):
                                me.image(src=image2_url, style=WINNER_THUMBNAIL_STYLE if winner == model2 else THUMBNAIL_STYLE)
                                me.text(model2, style=me.Style(font_size="10pt"))
                        me.html(html=f"{timestamp}: {model1} vs. {model2}:  <strong>{winner}</strong> won.")
